...
public partial class UserControl_oRejectedRequests : aspnetUserControl
{
    //long lRequestId = 0;
    ...
    protected void grdView_FilterCheckListItemsRequested(object sender, GridFilterCheckListItemsRequestedEventArgs e)
    {
        ...
        if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.Approver) GetDataTable = RequestHelper.dtRequestsIDidNotApprove(oSessnx.getOnlineUser.m_iUserId, DataField);
        else if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.Owner) GetDataTable = RequestHelper.dtMyRejectedRequests(oSessnx.getOnlineUser.m_iUserId, DataField);
        else if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.AllRequests) GetDataTable = RequestHelper.dtRejectedRequests(DataField);
    ...
    protected void grdView_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        if (!e.IsFromDetailTable)
        {
            ...
            if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.Approver) (sender as RadGrid).DataSource = RequestHelper.dtRequestsIDidNotApprove(oSessnx.getOnlineUser.m_iUserId, DataField);
            else if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.Owner) (sender as RadGrid).DataSource = RequestHelper.dtMyRejectedRequests(oSessnx.getOnlineUser.m_iUserId, DataField);
            else if (int.Parse(RequestorTypeHF.Value) == (int) commonEnums.RequestorType.AllRequests) (sender as RadGrid).DataSource = RequestHelper.dtRejectedRequests(DataField);
        }
    }

    public void My_Requests_Rejected(int iRequestorType)
    {
        try
        {
    ...
    public void Requests_I_Rejected(int iRequestorType)
    {
        try
        {
            RequestorTypeHF.Value = iRequestorType.ToString();
    ...
    public void Rejected_Requests(int iRequestorType)
    {
        try
        {
            RequestorTypeHF.Value = iRequestorType.ToString();
    ...
    protected void grdView_ItemCreated(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (viewCommentLink != null)
            {
                ...
                viewCommentLink.Attributes["onclick"] = string.Format("return ShowCommentFormRejected('{0}','{1}');", e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["IDREQUEST"], e.Item.ItemIndex);
            }
