<%@ Control Language="C#" AutoEventWireup="true" CodeFile="oIPApprovedRequests.ascx.cs" Inherits="UserControl_oIPApprovedRequests" %>
...
<telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
    <script type="text/javascript">
        ...
        function ShowCommentForm2(id, rowIndex) {
            var grid = $find("<%= grdView.ClientID %>");

            var rowControl = grid.get_masterTableView().get_dataItems()[rowIndex].get_element();
            grid.get_masterTableView().selectItem(rowControl, true);

            var wndw = window.radopen("ViewComments2.aspx?RequestId=" + id, "CommentVMDialog", 900, 500);
            wndw.set_visibleStatusbar(false);
            wndw.Center();
            return false;
        }

        function ShowReportForm(id, rowIndex) {
            var grid = $find("<%= grdView.ClientID %>");
        ...
        function RowDblClick(sender, eventArgs) {
            var wndw = window.radopen("ViewComments2.aspx?lRequestId=" + eventArgs.getDataKeyValue("IDREQUEST"), "CommentVMDialog", 900, 500);
            wndw.set_visibleStatusbar(false);
            wndw.Center();
        }
...
<div id="demo" class="demo-container no-bg" style="width: 100%">
    ...
    <telerik:RadWindowManager RenderMode="Lightweight" ID="RadWindowManager1" runat="server" EnableShadow="true">
        <Windows>
            <telerik:RadWindow RenderMode="Lightweight" ID="CommentVMDialog" runat="server" Title="View Comments" Height="500px"
